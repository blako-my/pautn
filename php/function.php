...
function deletedirectory($directorypath,$redirect = null)
{
    ...
    foreach ($files as $file) {
        $currentPath = $directorypath . $file;
...
function updateservice($conn,$userid,$service)
{
    $stmt = $conn->prepare("SELECT T1_service FROM T1_user WHERE T1_id = ?");
    $stmt->bind_param("s",$userid);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    $stmt->close();
    $current_services = json_decode($row['T1_service'], true) ?? [];
    $updated_services = array_merge($current_services, $service);
    ?><script>alert('<?php echo($updated_services);?>')</script><?php
    $addservice = json_encode($updated_services);
    $ustmt = $conn -> prepare("UPDATE T1_user SET T1_service = ? WHERE T1_id = ?");
    $ustmt -> bind_param("ss",$addservice,$userid);
    $success = $ustmt->execute();
    $ustmt->close();
    return $success;
}
?>
